...
ggsave(plot_density_regression, file = "plots/plot_density_regression.jpg", width = 6.5, height = 8)


# capture probabilities -----------------------------------------------

capture_prob_data = mod_d %>% 
...
capture_probs_taxa = mod_d %>% 
  ...
  left_join(capture_prob_data %>% ungroup %>% distinct(taxon, taxon_order))
...
plot_capture_probs = capture_probs %>% 
  ggplot(aes(x = site_f, y = 1 - zi, fill = taxon)) +
  geom_line(data = . %>% filter(.draw <= 500),
            aes(group = .draw, color = taxon), alpha = 0.02) + 
